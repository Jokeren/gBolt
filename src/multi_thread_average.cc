...
namespace gspan {
	bool GSpan::split(ProjectionMap& projection_map)
	{
		...
		for (size_t i = 0; i < THREAD_NUM; ++i) {
			_m_output[i] = new Output(_m_output_name, i);
		}
		...
		for (ProjectionMap::reverse_iterator it = projection_map.rbegin(); it != projection_map.rend(); ++it) {
			...
			_m_split_average[idx_output].push_back(it);

			idx_output = (idx_output + 1) % THREAD_NUM;
		}
	...
	{
		int32_t prev_id = -1;
		for (size_t i = 0; i < _m_split_average[tid].size(); ++i) {
			//another parital pruneing, like apriori
			ProjectionMap::reverse_iterator it = _m_split_average[tid][i];
