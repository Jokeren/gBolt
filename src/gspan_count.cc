...
namespace gspan {
	...
	void GSpan::build_graph(Graph& graph, size_t tid)
	{
		uint32_t edge_id = 0;
		Vertice vertice;
		...
		for (size_t i = 0; i < _m_dfs_codes[tid].size(); ++i) {
			...
			vertice.resize(std::max(static_cast<size_t>(_m_dfs_codes[tid][i].from + 1), 
						vertice.size()));
			vertice.resize(std::max(static_cast<size_t>(_m_dfs_codes[tid][i].to + 1),
					   	vertice.size()));

			//debug
			//printf("%u %u here1\n", _m_dfs_codes[i].from, _m_dfs_codes[i].to);

			vertice[_m_dfs_codes[tid][i].from].label = _m_dfs_codes[tid][i].from_label;
			vertice[_m_dfs_codes[tid][i].from].id = _m_dfs_codes[tid][i].from;
			vertice[_m_dfs_codes[tid][i].to].label = _m_dfs_codes[tid][i].to_label;
			vertice[_m_dfs_codes[tid][i].to].id = _m_dfs_codes[tid][i].to;

			//debug
			//printf("%u %u here2\n", _m_dfs_codes[i].from, _m_dfs_codes[i].to);
			struct edge_t edge;
			edge.from = _m_dfs_codes[tid][i].from;
			edge.to = _m_dfs_codes[tid][i].to;
			edge.label = _m_dfs_codes[tid][i].edge_label;
			edge.id = edge_id;
			++edge_id;
			//debug
			//printf("%u %u here3\n", edge.from, edge.to);
			...
			//printf("%u %u here4\n", _m_dfs_codes[i].from, _m_dfs_codes[i].to);
			//second edge
			edge.from = _m_dfs_codes[tid][i].to;
			edge.to = _m_dfs_codes[tid][i].from;
	...
	bool GSpan::is_min(size_t tid)
	{
		//debug
#ifdef DEBUG
		printf("is_min size %zu\n", _m_dfs_codes[tid].size());
#endif

		if (_m_dfs_codes[tid].size() == 1)
			return true;

		_m_min_graph[tid].clear();

#ifdef DEBUG
		printf("is_min here0\n");
#endif
		build_graph(_m_min_graph[tid], tid);

		_m_min_dfs_codes[tid].clear();
		ProjectionMap projection_map;

		for (size_t j = 0; j < _m_min_graph[tid].size(); ++j) {
			const struct vertex_t& vertex = _m_min_graph[tid].get_vertex(j);	
			Edges edges;	

			if (get_forward_init(vertex, _m_min_graph[tid], edges)) {
				for (size_t k = 0; k < edges.size(); ++k) {
					struct dfs_code_t dfs_code;
					//push dfs code according to the same edge label
					const struct vertex_t& vertex_from = _m_min_graph[tid].get_vertex(edges[k]->from);
					dfs_code.from_label = vertex_from.label;
					dfs_code.from = 0;

					dfs_code.edge_label = edges[k]->label;

					const struct vertex_t& vertex_to = _m_min_graph[tid].get_vertex(edges[k]->to);
					dfs_code.to = 1;
		...
		_m_min_dfs_codes[tid].push_back((struct dfs_code_t) {0, 1, 
				(it->first).from_label, (it->first).edge_label, (it->first).to_label});

		if (_m_dfs_codes[tid][_m_min_dfs_codes[tid].size() - 1] != _m_min_dfs_codes[tid][_m_min_dfs_codes[tid].size() - 1])
			return false;

		return projection_min(it->second, tid);
	}
			...
			for (size_t j = 0; j < projection.size(); ++j) {
				...
				for (size_t k = 0; k < last_node.edges.size(); ++k) {
					...
					if (last_node.edges[k].to == edge->from && 
						...
						uint32_t from_id = _m_min_dfs_codes[tid][right_most_path[0]].to;
						uint32_t to_id = _m_min_dfs_codes[tid][right_most_path[i - 1]].from; 
			...
			for (size_t i = 0; i < right_most_path.size(); ++i) {
				for (size_t j = 0; j < projection.size(); ++j) {
					...
					for (size_t k = 0; k < cur_node.edges.size(); ++k) {
						const struct vertex_t& to_node = _m_min_graph[tid].get_vertex(cur_node.edges[k].to);
						...
						if (cur_edge->label < cur_node.edges[k].label ||
							...
							uint32_t from_id = _m_min_dfs_codes[tid][right_most_path[i]].from;
							uint32_t to_id = _m_min_dfs_codes[tid][right_most_path[0]].to;
							struct dfs_code_t dfs_code;
	...
	bool GSpan::projection_min(const Projection& projection, size_t tid)
	{
		std::vector<uint32_t> right_most_path;
		build_right_most_path(_m_min_dfs_codes[tid], right_most_path);


		ProjectionMapBackward projection_map_backward;

		uint32_t min_label = _m_min_dfs_codes[tid][0].from_label;

		if (judge_backward(right_most_path, projection, projection_map_backward, min_label, tid)) {
			ProjectionMapBackward::iterator it = projection_map_backward.begin();	

			_m_min_dfs_codes[tid].push_back((struct dfs_code_t) {(it->first).from, (it->first).to, 
					(it->first).from_label, (it->first).edge_label, (it->first).to_label});
		...
		if (judge_forward(right_most_path, projection, projection_map_forward, min_label, tid)) {
