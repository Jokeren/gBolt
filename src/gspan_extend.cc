...
void GSpan::enumerate(
  ...
  for (size_t i = 0; i < projection.size(); ++i) {
    ...
    history_->build(projection[i], graph);

    get_backward(projection[i], graph, right_most_path, projection_map_backward);
    get_first_forward(projection[i], graph, right_most_path, min_label, projection_map_forward);
    get_other_forward(projection[i], graph, right_most_path, min_label, projection_map_forward);
  }
}
...
void GSpan::get_backward(
  ...
  for (size_t i = right_most_path.size(); i > 1; --i) {
    ...
    for (size_t j = 0; j < (last_node->edges).size(); ++j) {
      if (history_->has_edges((last_node->edges[j]).id))
        continue;
      if (!history_->has_vertice((last_node->edges[j]).to))
        continue;
...
void GSpan::get_other_forward(
  ...
  for (size_t i = 0; i < right_most_path.size(); ++i) {
    ...
    for (size_t j = 0; j < cur_node->edges.size(); ++j) {
      ...
      if (history_->has_vertice(to_node->id) || to_node->id == cur_to->id || to_node->label < min_label)
        continue;
